...
directorio_imagenes="/home/usuario/TP-ENTORNO/imagenes"
...
for archivo in "$directorio_imagenes"/*; do
  ...
  # Verificar si el nombre del archivo cumple con el patrón de una persona válida
  if [[ $nombre_base =~ ^[A-Z][a-z]+ ]]; then
    ...
    convert "$archivo" -gravity center -resize 512x512+0+0 -extent 512x512 "$archivo_salida"
